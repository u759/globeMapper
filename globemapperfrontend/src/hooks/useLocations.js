            ...
            try {
                if (date) {
                    const dateResponse = await fetch(
                        `https://138.2.226.216:4443/api/events/within-week?date=${date}`
                    );
                    if (!dateResponse.ok) {
                    ...
                    if (!dateData || dateData.length === 0) {
                        ...
                        const allResponse = await fetch("https://138.2.226.216:4443/api/events/all");
                        if (!allResponse.ok) {
                            throw new Error("Network response was not ok");
                        }
                    ...
                    const response = await fetch("https://138.2.226.216:4443/api/events/all");
                    if (!response.ok) {
                        throw new Error("Network response was not ok");
                    }
