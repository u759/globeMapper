            ...
            try {
                if (date) {
                    const dateResponse = await fetch(
                        `https://news.ayng.dev:2083/api/events/within-week?date=${date}`
                    );
                    if (!dateResponse.ok) {
                    ...
                    if (!dateData || dateData.length === 0) {
                        ...
                        const allResponse = await fetch("https://news.ayng.dev:2083/api/events/all");
                        if (!allResponse.ok) {
                            throw new Error("Network response was not ok");
                        }
                    ...
                    const response = await fetch("https://news.ayng.dev:2083/api/events/all");
                    if (!response.ok) {
                        throw new Error("Network response was not ok");
                    }
